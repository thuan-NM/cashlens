...
| After consent, the Email page shows **Không thể kết nối Gmail** | The address had `?gmail=failed&reason=…`: `STATE_INVALID` (expired or foreign flow), `GOOGLE_REFUSED` (code refused, or offline access not granted), `GOOGLE_UNAVAILABLE`, or `FAILED`. `gmail.callback_failed` in the log has the HTTP status and error class. | Act on the reason as in the rows above, then click **Kết nối lại Gmail** |
| After consent, the browser lands on the wrong site or port | The callback returns to the first `CORS_ORIGIN` | Set `CORS_ORIGIN` to the web app's origin |
