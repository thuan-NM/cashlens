  ...
  return (
    <div className="relative mt-7 max-w-[380px]">
      ...
      <div className="animate-float relative rounded-[20px] border border-white/20 bg-white/10 p-5 text-white shadow-[0_30px_60px_-24px_rgba(0,0,0,.55)] backdrop-blur-2xl">
        ...
        <div className="mt-3 text-[30px] font-bold tracking-tight tabular-nums">+7.200.000đ</div>
        ...
        <div className="space-y-3 border-t border-white/15 pt-3.5 text-[11.5px]">
          {[["Highlands Coffee", "Cà phê · TCB", "−78.000đ", "#D97757"], ["Lương tháng 6", "Thu nhập · VCB", "+24.500.000đ", "#62b88c"]].map((item) => (
...
export function AuthPage() {
  ...
  const submit = async (values?: { name?: string; email?: string; password?: string }) => {
    ...
    try {
      if (mode === "register") {
        await register({ email, password, fullName: values?.name });
        message.success("Tài khoản đã được tạo. Hãy đăng nhập để tiếp tục.");
        setMode("login");
        return;
      }
      ...
      message.error(error instanceof Error ? error.message : "Không thể xác thực tài khoản");
    }
  };
