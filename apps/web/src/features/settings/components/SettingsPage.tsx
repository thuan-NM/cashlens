...
export function SettingsPage() {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <Card><SectionHeading title="Tự động hóa" description="Kiểm soát cách hệ thống xử lý dữ liệu" /><div className="divide-y divide-[var(--border)]">{[["autoClassificationEnabled", "Phân loại tự động", "Rule + mô hình phân loại"], ["allowAiInsights", "AI insight", "Phân tích dữ liệu đã tổng hợp"], ["notificationEnabled", "Thông báo", "Nhận cảnh báo trong ứng dụng"]].map(([key, title, desc]) => <div key={key} className="flex items-center justify-between py-3"><div><div className="text-[11.5px] font-medium">{title}</div><div className="text-[10px] text-[var(--faint)]">{desc}</div></div><Switch checked={Boolean(settings[key])} onChange={(checked) => updateSetting({ [key]: checked })} /></div>)}
